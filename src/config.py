...
def load_config(page=None):
    ...
    Returns dict with keys 'api_url' and 'secret_key', or None if not found.
    """
    url = os.environ.get("CADUX_API_URL")
    key = os.environ.get("CADUX_SECRET_KEY")

    if not url or not key:
        load_dotenv(override=False)
        url = url or os.environ.get("CADUX_API_URL")
        key = key or os.environ.get("CADUX_SECRET_KEY")

    if (not url or not key) and page is not None:
        try:
            url = url or page.session.store.get("api_url")
            key = key or page.session.store.get("secret_key")
        except AttributeError:
            pass

    if url and key:
        return {"api_url": url, "secret_key": key}
